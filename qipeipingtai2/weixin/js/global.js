...
var imgUploadImg1 = imgUrl+'/api.sev.upload/uploadImgNoLogin';

var cardBg = '../../../image/card-bg.jpg';//名片背景
 ...
 *mui监听li点击事件
 */
mui("#conList").on('tap', 'ul', function (event) {
	this.click();
});

mui("#conList").on('tap', 'li', function (event) {
	this.click();
});

mui("#conList").on('tap', 'div', function (event) {
	this.click();
});

mui("#conList").on('tap', 'span', function (event) {
	this.click();
});

mui("#conList").on('tap', 'img', function (event) {
	this.click();
});
/**
 * 打开新页面
...
http.load = function(mod,act,data,funOk,funEr){
	var url = '/?m='+mod+'&a='+act;
		...
		http.load(mod,fun,postData,function(rData){//请求成功
 			...
 			if(rData.status==200){	    
		        ...
		        if(count<=showLen){//加载动图隐藏
		        	...
		        	//mui('#loadList').pullRefresh().endPullupToRefresh(false); //参数为true代表没有更多数据了。
		        	$("#page").val(page+1);
		        }
		        //如果条数为 0 出现提示语言
		        if(count==0&&page==1){
 				...
 				sw.toast(rData.msg);
				mui('#pullrefresh').pullRefresh().endPullupToRefresh(true);//隐藏加载提示
 			}
...
UserInfo.clear = function(){
	JsonStorage.removeItem('username');
	JsonStorage.removeItem('password');
...
UserInfo.username = function(){
	...
	return JsonStorage.getItem('username');
	}
	if(arguments[0] === ''){
	JsonStorage.removeItem('username');
	return;
	}
	JsonStorage.setItem('username', arguments[0]);
};

UserInfo.password = function(){
	if(arguments.length == 0){
	return JsonStorage.getItem('password');
	}
	if(arguments[0] === ''){
	JsonStorage.removeItem('password');
	return;
	}
	JsonStorage.setItem('password', arguments[0]);
};

UserInfo.token = function(){
	if(arguments.length == 0){
	return JsonStorage.getItem('token');
	}
	if(arguments[0] === ''){
	JsonStorage.removeItem('token');
	return;
	}
	JsonStorage.setItem('token', arguments[0]);
};
...
/**
 * 获取当前url中参数 以对象形式返回 url传递的参数一律以json字符串 参数名 data;
 * @returns {{}}
 */
sw.getQueryVariable = function()
{
	...
	return JSON.parse(theRequest.data);
};
