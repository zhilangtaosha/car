    ...
    public function getAllListZuoBiao($token,$city,$district,$classType,$jiShu,$shaiXuan,$page,$pageSize,$lat=30.674024,$lng=104.072315,$cityCode,$keywords){
        ...
        if($user){
            ...
            if($area == $cityCode){
                ...
                $sqlC .= '(SELECT '.$filedC.' FROM firms AS a LEFT JOIN firms_call_log AS b ON a.id = b.firms_id AND b.create_time>"'.$startTime.'" where a.province="'.$cityCode.'" GROUP BY a.id)';
                ...
                $sql .= '(SELECT '.$filed.' FROM firms AS a LEFT JOIN firms_call_log AS b ON a.id = b.firms_id AND b.create_time>"'.$startTime.'" where a.province="'.$cityCode.'" GROUP BY a.id)';
